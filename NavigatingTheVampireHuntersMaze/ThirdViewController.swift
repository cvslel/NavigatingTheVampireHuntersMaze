...
class ThirdViewController: UIViewController {
    ...
    func createUI(){
        ...
        imageViewHead.snp.makeConstraints { make in
            make.top.equalTo(imageViewMaze.snp.bottom).offset(-395)
            make.left.equalTo(view.snp.left).offset(290)
    ...
    @objc func downButtonPressed(){
        navigationController?.heroNavigationAnimationType = .selectBy(presenting: .push(direction: .right),
                                                                      dismissing: .push(direction: .left))
